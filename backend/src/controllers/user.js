...
const genrateToken = async (_id) => {
    try {
        ...
        console.log(
            `Error while genrating token id : ${_id} \n Error : ${error}`
        );
    }
};
...
export const registerUser = asyncHandler(async (req, res) => {
    ...
    const echoUser = await User.findById(newUser._id).select(
    ...
    res.status(201).json(
        new ApiResponse(
            201,
            echoUser,
            `User created having username ${echoUser.username}`
        )
    );
});
...
export const editpassword = asyncHandler(async (req, res) => {
    ...
    await user.save();
    const echoUser = await User.findById(user._id).select(
        "firstName lastName email username profileImage"
    );
    return res
        .status(200)
        .json(new ApiResponse(200, echoUser, "Password updated"));
});
